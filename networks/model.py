...
#     def forward(self, x):
        ...
        weight_ori = weights
        weights = None
# 如果 weight_ori 是 tensor，确保它在 CPU 上
        if isinstance(weight_ori, torch.Tensor):
            weight_ori = weight_ori.cpu().numpy()  # 将 tensor 移到 CPU 并转换为 numpy 数组

    # 如果 weight_ori 是 list，转换为 numpy 数组
        if isinstance(weight_ori, list):
            weight_ori = np.array(weight_ori)  # 将 list 转换为 numpy 数组

    # 如果是 numpy 数组，转换为 tensor
        weight_ori = torch.from_numpy(weight_ori).to(DEVICE)  # 转换为 tensor，并移动到指定的设备
